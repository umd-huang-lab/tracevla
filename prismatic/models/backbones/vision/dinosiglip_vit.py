...
class DinoSigLIPViTBackbone(VisionBackbone):
    def __init__(self, vision_backbone_id: str, image_resize_strategy: str, default_image_size: int = 224) -> None:
        ...
        self.sep_tok = torch.nn.Embedding(1, self.embed_dim)
    ...
    def forward(self, pixel_values: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        batch_size, num_steps, num_channel, img_height, img_width = pixel_values["dino"].shape
        pixel_values['dino'] = pixel_values["dino"].reshape(batch_size*num_steps, num_channel, img_height, img_width)
        pixel_values['siglip'] = pixel_values["siglip"].reshape(batch_size*num_steps, num_channel, img_height, img_width)
        
        dino_patches = self.dino_featurizer(pixel_values["dino"])
        siglip_patches = self.siglip_featurizer(pixel_values["siglip"])

        patches = torch.cat([dino_patches, siglip_patches], dim=2)
        patches = patches.reshape(batch_size, num_steps, -1, self.embed_dim)
