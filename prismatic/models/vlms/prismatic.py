...
# Custom Logits Processor to Constrain Output Tokens
class ActionTokenLogitsProcessor(LogitsProcessor):
    def __init__(self, action_start_idx, action_end_idx):
        self.action_start_idx = action_start_idx
        self.action_end_idx = action_end_idx

    def __call__(self, input_ids, scores):
        # Zero out logits for tokens outside the action range
        mask = torch.ones_like(scores, dtype=torch.bool)
        mask[..., self.action_start_idx:self.action_end_idx + 1] = False
...
class PrismaticVLM(VLM):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if "vision_backbone" in model_state_dict.keys():
            if 'sep_tok.weight' in model_state_dict["vision_backbone"]:
    ...
    @torch.inference_mode()
    def generate_actiontokens(self, image: Image, prompt_text: str, action_tokenizer: ActionTokenizer, **kwargs: str) -> str:
        # For now, only support generation with a batch size of 1 for simplicity
        ...
        # Create a custom logits processor
        logits_processor = LogitsProcessorList()
        logits_processor.append(ActionTokenLogitsProcessor(action_start_idx, action_end_idx))

        # Invoke super().generate with the custom logits processor
        autocast_dtype = self.llm_backbone.half_precision_dtype
        with torch.autocast("cuda", dtype=autocast_dtype, enabled=self.enable_mixed_precision_training):
            # fmt: off
            generated_ids = super().generate(
                ...
                logits_processor=logits_processor,  # Pass the custom logits processor
                **kwargs
            )
            # fmt: on
