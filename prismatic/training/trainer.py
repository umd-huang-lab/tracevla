...
class VLATrainer:
    ...
    def __init__(
        ...
        self.action_loss_calculation = action_loss_calculation
...
#         return self.accelerator.prepare(DataLoader(train_dataset, **dataloader_params))
    def get_train_dataloader(self) -> DataLoader:
        ...
        worker_init_fn = set_global_seed(self.args.seed, get_worker_init_fn=True)
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            optimizer_cls, optimizer_kwargs = VLATrainer.get_optimizer_cls_and_kwargs(self.args, opt_model)
    ...
    def _inner_training_loop(
        ...
        tr_action_accuracy = torch.tensor(0.0).to(args.device)
        tr_action_l1_loss = torch.tensor(0.0).to(args.device)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with self.accelerator.accumulate(model):
                    tr_loss_step, tr_action_accuracy_step, tr_action_l1_loss_step = self.training_step(model, inputs)
                ...
                if (
                    ...
                    tr_action_accuracy += tr_action_accuracy / (1 + self.state.global_step - self._globalstep_last_logged)
                    tr_action_l1_loss += tr_action_l1_loss / (1 + self.state.global_step - self._globalstep_last_logged)
                else:
                    ...
                    tr_action_accuracy += tr_action_accuracy_step
                    tr_action_l1_loss  += tr_action_l1_loss_step
                ...
                if (
                    ...
                    self._maybe_log_save_evaluate(tr_loss, tr_action_accuracy, tr_action_l1_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval)
                else:
    ...
    def _maybe_log_save_evaluate(self, tr_loss, tr_action_accuracy, tr_action_l1_loss, 
                                 grad_norm, model, trial, epoch, ignore_keys_for_eval):
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            tr_action_accuracy_scalar = self._nested_gather(tr_action_accuracy).mean().item()
            tr_action_l1_loss_scalar = self._nested_gather(tr_action_l1_loss).mean().item()
            
            # reset tr_loss to zero
            tr_loss -= tr_loss
            tr_action_accuracy -= tr_action_accuracy
            tr_action_l1_loss -= tr_action_l1_loss

            logs["loss"] = round(tr_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["action_prediction_accuracy"] = round(tr_action_accuracy_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["action_l1_loss"] = round(tr_action_l1_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        with self.compute_loss_context_manager():
            loss, action_accuracy, action_l1_loss = self.compute_loss(model, inputs)

        if self.args.n_gpu > 1:
            loss = loss.mean()  # mean() to average on multi-gpu parallel training
            action_accuracy = action_accuracy.mean().detach()
            action_l1_loss = action_l1_loss.mean().detach()
        ...
        return loss.detach() / self.args.gradient_accumulation_steps, action_accuracy / self.args.gradient_accumulation_steps, action_l1_loss / self.args.gradient_accumulation_steps
    
    def compute_action_token_loss(self, logits, labels, action_start_idx, n_bins=256):
        batch_size, seq_len, vocab_size = logits.shape

        # Define the range of action tokens
        action_end_idx = action_start_idx + n_bins - 1

        # Mask to select only the action token logits
        action_logits = logits[..., action_start_idx:action_end_idx + 1]  # Keeps the shape as (batch_size, seq_len, 256)

        # Create a mask to identify labels within the action token range
        mask = (labels >= action_start_idx) & (labels <= action_end_idx)

        # Adjust labels only if within the action token range
        adjusted_labels = labels.clone()
        adjusted_labels[mask] = labels[mask] - action_start_idx

        # Set ignored labels outside the action token range
        adjusted_labels[~mask] = -100  # Assuming -100 is the ignore index for CrossEntropyLoss

        # Calculate loss over the action tokens without flattening
        loss_fct = torch.nn.CrossEntropyLoss(ignore_index=-100)  # Set ignore_index to match your ignore index
        loss = loss_fct(action_logits.permute(0, 2, 1), adjusted_labels)  # Permute logits to (batch_size, vocab_size, seq_len)
        return loss
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        logits = outputs["logits"][:, : -1].cuda()
        labels = inputs["labels"][:, 1:].cuda()
        
        
        action_start_idx = self.action_tokenizer.action_token_begin_idx + 1
        action_end_idx = self.action_tokenizer.action_token_begin_idx + 256

        # Extract logits for only the action tokens
        if self.action_loss_calculation == 'full':
            action_preds = logits.argmax(dim=2)
        elif self.action_loss_calculation == 'action_token_only':
            action_logits = logits[..., action_start_idx:action_end_idx + 1]
            action_preds = action_logits.argmax(dim=2)
            action_preds = action_preds + action_start_idx
        else:
            raise ValueError(
                    "Action loss calculation is not supported!"
                )
        ...
        if self.action_loss_calculation == 'full':
            ...
            loss = self.compute_action_token_loss(logits, labels, action_start_idx)
        else:
            raise ValueError(
                    "Action loss calculation is not supported!"
                )
        
        return (loss, action_accuracy, action_l1_loss, outputs) if return_outputs else loss, action_accuracy, action_l1_loss
