...
import numpy as np
import torch
from grootcommon.gym.base_vector_policy import BaseVectorPolicy, get_batch_from_info
...
class OpenVLAPhi3Inference(BaseVectorPolicy):
    ...
    def unnormalize_action(self, normalized_actions, i):

        assert len(normalized_actions) == 7, f"{len(normalized_actions)=}"
        if U.global_n_times("normalized_actions", 10):
            print(f"{normalized_actions=}")

        # Unnormalize actions
        action_norm_stats = self.get_action_stats(self.unnorm_keys[i])
        ...
        if self.policy_setups[i] == "widowx_bridge":
            ...
            if self.previous_gripper_actions[i] is None:
                ...
                relative_gripper_action = self.previous_gripper_actions[i] - current_gripper_action
    ...
    def get_action(
        ...
        for i in range(self.num_envs):
            ...
            ### This is because our dataset is stored at 256x256 resolution.
            ### So during training, it is first resized to 256x256,
            ### and then resized to 336x336. Here we aim to exactly match the training setup.
            ...
            inputs = self.processor(prompt, [image]).to(device=self.vla.device, dtype=torch.bfloat16)
