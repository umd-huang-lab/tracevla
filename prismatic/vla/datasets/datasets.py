...
class RLDSBatchTransform:
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not traced or np.random.uniform() < self.random_drop_rate:
            ...
            images = [Image.fromarray(image) for image in images]
            conversation = [
                 {"from": "human", "value": f"You are given two images: one with the original robot observation, and another one marked with historical traces of the robot end effector and moving objects, separated by a special separator token. What action should the robot take to {lang}?"},
                 {"from": "gpt", "value": self.action_tokenizer(action)},
            ]
